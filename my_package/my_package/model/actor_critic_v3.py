...
class Actorv3(nn.Module):
    ...
    def forward(self, h, cls = None):
        ...
        if cls is None:
            ...
            h = torch.cat((self._fc1(cls).unsqueeze(0), h), dim = 0)
        h = self.layer1(h)
...
if __name__ == "__main__":
    state_dim = 11
    x = torch.randn((200, state_dim))
    model = Actorv3(state_dim, 20)
    with torch.no_grad():
        results = model(x)
    print(results.shape)
